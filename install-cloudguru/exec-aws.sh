...
subnetid=$(aws ec2 describe-subnets  --profile ${profile} --filters "Name=availability-zone,Values=us-east-1a" --query "Subnets[0].SubnetId" | sed -e 's/^"//' -e 's/"$//')
echo Subnet id = $subnetid

#get default VPC security group
sg=$(aws ec2 describe-security-groups  --profile ${profile} --query "SecurityGroups[?GroupName=='default'].GroupId | [0]" | sed -e 's/^"//' -e 's/"$//')
echo Default Security Group = $sg
...
then
  rm -f $keypair
  aws ec2 create-key-pair  --profile ${profile} --key-name pgmSSHKey --query 'KeyMaterial' --output text >$keypair
  chmod 400 $keypair

  aws cloudformation create-stack  --profile ${profile} --stack-name ${tier}-ec2-serverless --template-url "https://${s3bucket}.s3.amazonaws.com/${cf_ec2}" --parameters ParameterKey=VpcId,ParameterValue=$vpcid ParameterKey=SubnetId,ParameterValue=$subnetid ParameterKey=WebSGId,ParameterValue=$sgid
  echo -n "Creating stacks..."
  webserver=null
  while [ ${webserver} == null ]; do
      sleep 2s
      echo -n "."
      webserver=$(aws cloudformation describe-stacks  --profile ${profile} --stack-name ${dev}-ec2-serverless --query "Stacks[0].Outputs[?OutputKey=='WebServer'].OutputValue | [0]")
  done
