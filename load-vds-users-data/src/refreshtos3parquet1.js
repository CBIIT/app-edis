...
async function run()
{
    ...
    await batchUpload(queueUsers);
   
    await closeWriteStreams();
    console.log("Imported " + usersCounter + " data records into S3 bucket ");
}
...
let processedTotalCounter = 0;

async function batchUpload(queue) {
    if (table === 'T') {
    ...
    try {
        ...
        while (queue.length > 0) {
            ...
            while (typeof (user) !== 'undefined') {
                ...
                if (processedTotalCounter % 10000 === 0) {
                    console.info(processedTotalCounter, ' records written...');
                }
            }
            console.debug('Batch has been written', processedTotalCounter);
        }
    } catch (err) {
...
async function closeWriteStreams() {
    try {
