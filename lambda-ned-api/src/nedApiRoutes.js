...
var wsdl = '';
var wsdl_changes = ''; //WSDL to get NED Changes

function config(ned_username, ned_password, ned_wsdl, ned_wsdl_changes) {
    wsdl = ned_wsdl;
    wsdl_changes = ned_wsdl_changes;
    wsSecurity_v7 = new WSSecurity(ned_username, ned_password);
}
...
async function getByName(firstName, lastName) {
    ...
    console.debug(`Continue in real mode 1 `, wsdl);
...
async function _getSoapClientForChanges() {
    if (!soapClientForChanges) {
        soapClientForChanges = await soap.createClientAsync(wsdl_changes);
        soapClientForChanges.setSecurity(wsSecurity_v7);
    }
    return soapClientForChanges;
}

module.exports = {nedRoutes, config, getByName}